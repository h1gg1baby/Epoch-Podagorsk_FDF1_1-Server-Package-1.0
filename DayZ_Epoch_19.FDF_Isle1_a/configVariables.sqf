...
DZE_Butcher_Body = true; // Enable Butcher Bodies
...
if (!isDedicated) then {
	...
	if (Z_SingleCurrency) then {		
		...
		ZSC_ZombieCoins = [true,[0,1000]]; // First value activate coins on zombies, second value from 0 - 1000 coins on each zombie. Coin for zombies are handled directly in check wallet.
	};
	...
	*/
	...
	if (DZE_Service_Points) then {
		...
		if (DZE_SP_Repair_Enable) then {
			...
			DZE_SP_Repair_Costs = [
				["Air",2000], // 4000 worth is 4 10oz gold.
				["AllVehicles",1000] // 2000 worth is 2 10oz gold for all other vehicles
			];
		};
